...
| 3 | **任务图**：任务可写 `前置`；`course.md` 可写 `遍历模式` | 默认为线性，现有课程零改动（见 §9） |
...
- `遍历模式`：`sequential`（默认）、`open` 或 `inquiry`，见 §9。可省略。
...
- `sequential`——任务按顺序推进（现状，5 门课都是这个）；
- `open`——学生在所有已解锁任务间自选（依赖 `前置` 解锁）；**R3 本轮落地**；
- `inquiry`——无任务顺序，由 `methodology.md` 的方法论驱动对话；**本期未实现**，lint 报 `unsupported_traversal_mode` warning，运行时将按 `sequential` 处理。

**两条复杂度约束**：无序只发生在任务层，Step 内永远线性；闯关线只渲染 `sequential`，对话线支持三种。

本期默认只跑 `sequential`（5 门课实测均为该值）。`open` 字段现在就可以写；`inquiry` 写了会收到 lint 提示。
