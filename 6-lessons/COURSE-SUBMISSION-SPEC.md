...
- 絮絮的名称、人设、基础形象、待机动画和对话动画由平台 `PLATFORM_COMPANION` 提供，课程配置不能修改。
- 课程团队通过阶段提示、知识、引导与限制定义本课教学行为，不在 `course.md` 提交 `## 智能体人设`。
- 公开课程对象不包含 `persona`，其 `assets` 也不包含 `companionIdle` 或 `companionTalk`。
