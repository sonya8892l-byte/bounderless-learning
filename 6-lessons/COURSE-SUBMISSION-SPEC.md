...
| 3 | **任务图**：任务可写 `前置`；`course.md` 可写 `遍历模式` | 缺省为线性，现有课程零改动（见 §9） |
...
- `遍历模式`：`sequential`（缺省）、`open` 或 `inquiry`，见 §9。可省略。
...
需要调整脚手架语义或升档参数时写 `## 脚手架`，可用键与 `_platform/scaffolding.md` 相同：`L0`…`L4`、`最高等级`、`升档触发`、`缺省提示`。任务/小步里的脚手架表仍是具体话术的权威来源；本节只调平台级语义与参数。本阶段不支持降档。

### 工具缺省（可选）

整门课统一改工具显示名或缺省字段文案时写 `## 工具缺省`，键为工具 id（如 `photo`）或 `text.observation`。决定程序分支的中文匹配（如扫码器何时切物体识别）不在此覆盖。
...
- 可调侧面写在 `## 人设侧重`：`character`（性格底色整句替换）、`tone`（语气）、`口头禅`、`侧重`（一句话说明本课偏向什么语气）。缺省值见 `_platform/companion.md`。
...
三段都是可选的。写在 `#### Step N` 内部即属于该 Step；写在 `### 任务N` 的字段区之后、第一个 `#### Step` 之前即属于该任务，作为其所有 Step 的缺省。**Step 级优先于任务级**，两者不叠加。
...
- `open`——学生在所有已解锁任务间自选（依赖 `前置` 解锁）；
- `inquiry`——无任务顺序，由 `methodology.md` 的方法论驱动对话。

**两条复杂度约束**：无序只发生在任务层，Step 内永远线性；闯关线只渲染 `sequential`，对话线支持三种。

本期平台只实现 `sequential`，写 `open`/`inquiry` 暂不生效。字段现在就可以写，专项启动时数据已就位。
...
| `失败处理` | Step | — | **未接入**：待图执行器接入失败路由 |
| `教师介入` | Step | — | **未接入**：待教师端挂点 |
