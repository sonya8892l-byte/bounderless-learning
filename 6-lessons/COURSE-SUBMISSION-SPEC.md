...
> 版本：**v2.2**（2026-08-13 课程单文件命名空间更新）｜依据：2026-08-06 目标架构评审的 D1–D5 决策，以及 2026-08-13 对运行时权威顺序、任务三轴和课程源边界的复核
...
| 5 | **非角色任务**：不属于任何角色的集体／小组／个人任务写成 `course.md / ## 阶段编排` 的 `#### 阶段任务N` | 选择角色前的阶段任务**生效**；选择角色后的全班／小组共享轨道当前**不支持**（见 §0.3、§4.1） |
...
编译器仍可读取旧课程的外置 `objectives.md`、`phases.md`、`restrictions.md`，仅用于迁移存量课程。新课和已迁移课程只维护 `course.md` 中的 `## 课程目标体系`、`## 阶段编排`、`## 课程限制规则`；内外双源会视为配置错误。
...
3. `course.md`：保存课程基本信息、三个标准命名空间与白名单覆盖；没写可选覆盖章节就完整继承平台值。
4. `prompts/phaseN-*.md`：当前 Phase 的阶段目标、工作方式、禁止行为和转场条件。
5. 当前任务与 Step：`roles/*.md` 或 `course.md / ## 阶段编排` 中就地写的行动、工具、引导、脚手架和验收。
...
| `course.md` | 课程基本信息、角色选择体系、学习视图、视觉素材、平台默认的白名单覆盖，以及课程目标、阶段和限制三个标准命名空间 | 整份缺失则编译失败；某个可选覆盖章节缺失则继承平台默认 | 运行配置**生效／报错**；目标定义**记录** |
| `course.md / ## 阶段编排` | Phase 名称、时长、模式、地点；`#### 阶段任务N` | 没写阶段任务就没有正式非角色任务 | 展示与选择角色前任务**生效**；触发／结束为**记录**；角色后共享轨道**不支持** |
...
| `course.md / ## 课程限制规则` | 防剧透、角色隔离、保护词与解锁条件 | 缺失时仍有平台底线，但没有本课保护清单 | **生效** |
...
| `course.md / ## 课程目标体系` | DK/DS/DC 课程标签定义与教研说明 | 不进入浏览器、Prompt 或实时评价 | **记录** |
| `README.md`、`assets-checklist.md` | 课程团队维护手册、素材来源与授权台账 | 不影响课堂，不进入 `courseVersion`；不能代替文件存在性 lint | **记录** |
...
`courseVersion（课程源版本）`只纳入会被课程编译器消费的运行 Markdown。`course.md / ## 课程目标体系`虽不改变本轮对话，仍属于受解析的课程源，修改后会更新版本。`README.md`和`assets-checklist.md`是团队管理文档，不纳入该指纹，修改它们不会使课程会话或运行缓存失效。
...
| 选择角色前的阶段任务 | `course.md / ## 阶段编排 / #### 阶段任务N` | 系统从开场到角色任务阶段（含该 Phase）寻找第一条含任务的 Phase，给每个学生会话各自执行 | 没写就直接进入角色选择流程 | **生效** |
...
| 选择角色后的全班／小组阶段任务 | 理论上仍写 `course.md / ## 阶段编排 / #### 阶段任务N` | 当前学生端不会切入这条共享任务轨道 | 即使写了，也不能依赖它在角色任务之后出现 | **不支持** |
...
| 不能说的课程内容 | `course.md / ## 课程限制规则` + `限制引用` | 当前 Step 只注入精确命中的最小限制片段；lint、运行时和评测共用同一 canonical matcher，把阿拉伯／中文数字、万／亿量级、百分比、`m³`／立方米等写法归一，并拦截高置信等价结论；不对开放语义做模糊猜测 | **生效** |
...
1. `course.md / ## 阶段编排` 的 Phase 1 当前只有一项正式阶段任务：“查看暴雨将至情境图”，执行单位标为`全班`。当前仍由每个学生会话各自完成；“全班”还没有整班共享完成记录。
2. `roles/*.md` 有六个角色，每个角色三个正式角色任务，共十八个；选择角色后，每个学生进入自己角色的任务轨道。
3. Phase 3 的“六角色发现拼合”和 Phase 4 的“沙盘推演”属于角色选择后的共享阶段轨道，目前**不支持**自动切入。教师组织说明写入 README 的教研备忘。
4. `course.md` 没有`## 脚手架`是正确配置：整课沿用`_platform/scaffolding.md`的 L0–L4 语义、最高档、升档和兜底提示；六个角色文件里的`##### 脚手架`负责各任务的具体话术。两者分层明确，不存在不匹配。

放置原则：**只服务当前任务的内容就地写进任务或 Step；跨任务共享的知识放入 `knowledge/*.md`，限制放入 `course.md / ## 课程限制规则`，量规放入 `evaluation.md`；整门课确实需要偏离平台默认时才在 `course.md` 写白名单覆盖。课程叙事、选题池、成果设计和史料边界等作者说明放入 README 的“教研备忘（不参与运行）”。**
...
课程 lesson_xxx/
│
├─ Phase 阶段         course.md / ## 阶段编排 ＋ prompts/phaseN-*.md
...
| 写在哪 | `course.md / ## 阶段编排` | `roles/<role>.md` |
...
`course.md / ## 阶段编排`里的`触发条件`和`结束条件`当前会被解析并下发公开课程对象，但没有自动执行器消费；倒计时归零也不会根据它们自动切换`phaseId`。它们目前属于**记录**字段，教师仍需明确推进。

任务里那个 `- 阶段：Phase 2 现场采证` 字段**当前没有任何运行行为**。它会被解析保存，但没有任何代码读它——不做任务过滤，也不做阶段绑定。它今天的价值只是给人看的注释。想让任务和阶段真正联动，得等 §9 的图执行器。

### 0.3 非角色任务：写在 course.md / ## 阶段编排 的「阶段任务」
...
这三类在内容模型里都写成**阶段任务**，挂在`course.md / ## 阶段编排`的对应 Phase 下，写法见 §4.1。当前运行时只接入了第一类：选择角色前的阶段任务。后两类可以先作为课程设计记录，不能依赖学生端在角色任务结束后自动进入。
...
### 教研备忘（authoring-only，不参与运行）

课程叙事、选题池、成果设计、调研边界和史料边界等作者说明统一写入该课 README 的“教研备忘（不参与运行）”小节。README 不进入解析器、Prompt、公开课程包或 `courseVersion`。需要真实约束学生或 AI 的边界，必须写入 `course.md / ## 课程限制规则`、Phase Prompt 或 Step 的 `限制引用`。
...
只能改**说什么**，不能改**什么时候说**：流程、工具调用和推进节奏由引擎决定。占位符照抄平台模板里的写法，漏写会在学生面前显示成 `{location}` 这样的字面量。模板里出现 `course.md / ## 课程限制规则` 的保护词会导致课程编译失败。
...
| P04 时间管理 | 平台固定 | 阶段倒计时、暂停、教师加减时读取 `course.md / ## 阶段编排`；它与时间银行余额分别管理。 |
...
上述私有字段不得出现在生成的学生公开课程对象、DOM 属性、浏览器日志或静态 JSON 中。新增工具判定字段时，需同步加入服务端私有字段过滤和测试。课程的真值、参考值、正确答案与允许误差必须写入已定义的工具私有参数、限制规则、知识条目或验收标准，不能自创角色字段存放。公开字段应在卡片中完整显示；作者简洁预算用于提交前审阅和 lint，不得被实现成浏览器端省略号、切字或聊天气泡拆分。
...
阶段编排只保留可解析的 Phase 字段与阶段任务。教师组织说明放入 README 的“教研备忘（不参与运行）”，不混入课程源字段。

每个 Phase 的絮絮规则写在`prompts/phaseN-*.md`。每份文件使用且仅使用以下五个二级标题：

| 二级标题 | 普通回合是否注入 | 用途 |
|---|---|---|
| `## 阶段目标` | 是 | 当前阶段要形成的认识或产出 |
| `## 絮絮行为` | 是 | 本阶段怎样主持、提问、等待、回应和组织产出 |
| `## 开场白模板` | 首次进入 Phase 时显示一次 | 学生可见的本阶段开场话；原样显示，后续回合不重复 |
| `## 禁止行为` | 是 | 阶段专属禁区、来源、安全与角色边界 |
| `## 转场条件` | 是 | 模型何时可以说转场话术；不自动改变`phaseId` |

五个标题缺任意一项都会产生`phase_prompt_missing_core_section` warning。未知二级标题会产生`phase_prompt_unknown_section` warning并忽略。没有二级标题的旧文件会以全文完整装配，不做字符截断，同时产生`phase_prompt_unstructured` warning；这条路径属于**兼容**。

### 4.1 阶段任务（非角色任务）

在 `course.md` 中，命名空间使用 `## 阶段编排`，Phase 使用 `### Phase N：<名字>`，阶段任务使用 `#### 阶段任务N：<名字>`：
...
写了三值之外的词（比如`全组`）时，解析器产生`bad_phase_task_executor` warning并回落为`全班`，实际完成范围已经偏离作者意图。发布 lint 已直接读取`course.md / ## 阶段编排`原始字段并报`bad_phase_task_executor` error，不能依赖解析器回落上线。
...
| `限制引用` | `course.md#课程限制规则/核心路线限制` | 精确解析同名表格行或二、三级标题；最小匹配片段进入当前小步对话和 AI 验收 | 标题必须逐字一致；优先引用最小限制单元，避免把无关规则送入模型 |

`限制引用` 找不到精确标题时不会自动扩大到整份 `course.md / ## 课程限制规则`，编译器返回空匹配。发布 lint 已报`dead_restriction_ref` error并定位到 file:line；提交时仍应人工确认引用选中的限制片段是否足够小且语义正确。
...
8. `能力标签` 前缀合法（CC/CQ/DK/DS），DK/DS 编号在本课 `course.md / ## 课程目标体系` 中有定义；
...
| `DK-*` | 学科知识 | 本课 `course.md / ## 课程目标体系` | 课程 |
| `DS-*` | 学科能力 | 本课 `course.md / ## 课程目标体系` | 课程 |
| `DC-*` | 课程级核心能力 | 本课 `course.md / ## 课程目标体系` | 课程（过渡用，见下） |
...
| 阶段任务（`course.md / ## 阶段编排`） | **同一 Phase 内**的其他阶段任务 id | 任意角色任务、其他 Phase 的任务 |
...
| Phase `名称／时长／模式／地点` | `course.md / ## 阶段编排` | 学生端顶栏、倒计时和组织信息上下文 | **生效** |
| Phase `触发条件／结束条件` | `course.md / ## 阶段编排` | 编译与公开课程对象 | **记录**：不自动切换`phaseId` |
| `prompts/phaseN-*.md`五段结构化规则 | Phase | `session.phaseId`选择后进入 System Prompt；开场白在首次进入时原样显示一次 | **生效** |
...
| `#### 阶段任务N` | `course.md / ## 阶段编排` | `entryPhaseForLesson()`选择入口 Phase，再由无角色会话与学生端任务卡消费 | **生效**：只接入从开场到角色任务阶段（含该 Phase）的第一条含任务 Phase；角色后共享轨道**不支持** |
