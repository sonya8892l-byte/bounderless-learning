...
| 任务与 Step 就地引导 | 已从 `roles/*.md` 装配 | `##### 引导`按 Step 优先、任务兜底；本课不使用独立 `guidance/` 目录 |
| 任务与 Step 就地脚手架 | 已从 `roles/*.md` 装配 | `##### 脚手架`使用 L0–L4；本课不使用独立 `scaffolds/` 目录 |
...
| `知识引用`、`限制引用` | 已接入 | 知识按 ID 定向检索；限制按 `restrictions.md#标题` 精确解析 |
| `引导引用`、`脚手架引用`、`评估引用` | 已废止 | 解析器只保留兼容读取，不产生运行行为；迁移时删除并改为就地内容 |
...
以下内容是课程教研与实施说明，不作为运行时自动聚合、自动转场或自动解锁的承诺。
...
当前运行时按 Step 数组顺序推进；`通过后：step:<id>`不改变 Step 游标，跨任务边进入任务图。知识引用已用于当前小步检索；限制引用按 `restrictions.md#标题` 精确解析并进入对话与 AI 验收。引导、脚手架与 Step 验收标准从当前任务单元就地读取；三类旧引用不产生行为。
...
本课把任务专属教学策略直接写在对应任务或 Step 的 `##### 引导`中。Step 级内容优先于任务级内容；都缺失时只使用任务行动、证据要求和平台通用规则。必须执行的泄题边界仍写入 `restrictions.md`，场馆安全规则同时写入阶段 Prompt 和教师实施方案。
