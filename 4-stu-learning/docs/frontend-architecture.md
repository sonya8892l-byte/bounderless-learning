...
课程导入期会执行课程配置的选择前阶段任务；没有阶段任务时显示等待态。入口任务完成、教师开放领取后进入角色选择。正式场次中的角色由学生领取并在组内唯一占位，首次领取复用原阶段会话；本地预览保留自由切换。
...
- `phases.md`：课程阶段、时长、模式、地点和触发条件；另含**阶段任务**（非角色任务，`### 阶段任务N`）。选择角色前的第一条阶段任务轨道已经接入运行时；角色选择后的共享阶段任务暂未接入；
...
闯关页以角色任务为分页单位：已完成任务可只读回看，当前任务可操作，未来任务不开放任务页，其名称与锁定状态只在进度面板中展示。`challengePageIndex` 只控制已解锁任务的查看页，不能代表正式进度；任务完成后仍以服务端推进结果解锁下一项。页内 AI 反馈镜像同一轮 SSE 文本，并以随后的 `state.updated` 判断通过或继续修改；请求失败时保留草稿和原进度。

“平台验收模式”属于运行环境控制，不进入课程 Markdown，也不属于 `learningView`。本地 Vite 开发会自动显示“强制完成本关并进入下一关”；Preview 只有在服务端 `QA_FORCE_COMPLETE_ENABLED=true` 与浏览器构建变量 `VITE_QA_FORCE_COMPLETE_ENABLED=true` 同时开启时才可使用；Production 服务端无条件拒绝该接口。验收动作补齐当前 Step 与任务完成记录，再复用正式的 `advanceToNextTask()`，因此测试会话会真实解锁下一任务。它不会生成证据、定位、教师确认或 AI 评分，并在会话 `qaOverrides[]` 留下独立审计记录。
