 ...
 */
export function buildTaskGraph(roles = []) {
  const nodes = new Map();
  const warnings = [];
  ...
  const resolveTarget = (roleId, target) => {
    const key = nodeKey(roleId, target);
    return nodes.has(key) ? key : '';
  };

  for (const role of roles) {
    for (const task of role.tasks || []) {
      const node = nodes.get(nodeKey(role.id, task.id));
      ...
      if (declared.length) {
        ...
        for (const value of declared) {
          const target = resolveTarget(role.id, value);
      ...
      for (const step of task.steps || []) {
        ...
        if (kind === 'task') {
          const targetKey = resolveTarget(role.id, target);
          if (!targetKey) {
