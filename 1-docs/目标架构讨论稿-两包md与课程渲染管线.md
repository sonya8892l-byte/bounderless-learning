...
> **落地进度（2026-08-07）**：D1／D3／D5／**D6** 已实现——任务单元就地格式定稿、5 门存量课迁移完成、`guidance/`＋`scaffolds/` 目录取消、语义理解优先已接入 `runTurn`。D2（`前置` 为权威）字段就位但**无运行时读者**。偏差①（平台包残缺）仅完成标签树与覆盖白名单声明；偏差③（IR 缺位）未动。逐项状态见 [实施计划 §0.5](./问题清单-优先级与解决思路.md)。
...
### 2.2 偏差①：平台包残缺——"系统 md"基本不存在

你要"课程 md + 系统 md 渲染课程"，但目前系统侧真正 md 化的只有 3 份底线规则。**大量本应属于平台包的内容硬编码在 JS 里**：

| 应属平台包的内容 | 现在在哪 |
|---|---|
| 絮絮人设/性格/语气 | `platform-config.js` 常量（course.md 写的人设侧重**不生效**） |
| 学段表达规范（字数/句式） | `prompt.js` 的 `gradeDialoguePolicy()` 硬编码 |
| 流程话术（入场/导航/求助/完成确认…） | `service.js` 的 `workflowResult()` 约 15 个分支硬编码中文 |
| 脚手架 L0–L4 语义与升降策略 | `service.js` 散落（且只升不降、L0/L4 取不到） |
| 十种工具缺省参数/提示文案 | `tool-registry.js` |
| 时长/提醒/推进方式等缺省值 | `lesson-parser.js` 硬编码 |

**后果**：课程作者改不动语气与话术；教研调整平台缺省要改代码发版；"换课程只需 md"在系统侧不成立。这是对承诺1 最大的违背。
...
| `course.md` 人设侧重字段 | 写了不生效 | 平台 `companion.md` 定义"锁定核 + 可调侧面"，课程覆盖真正生效 |
...
| D2 | 进度语义的权威字段 | **`前置`（入边）为权威**——同时表达顺序与解锁，天然支持模式2；`通过后` 保留为线性课程的语法糖，编译期转换为前置关系 | 🟡 字段可写、解析入 IR 前身，**无运行时读者** |
| D3 | 就地/集中的边界 | **就地**：引导、脚手架、Step 验收标准；**集中**：知识、限制、课程级量规、时间银行 | ✅ 含 B4 根治 |
| D4 | 平台覆盖白名单 | **不可覆盖**：三底线、絮絮名字/底色；**可覆盖**：语气侧面、话术模板、学段规范、工具缺省、时长缺省 | 🟡 已在 `_platform/README.md` v2 声明格式，`mergeDefaults` 未实现 |
