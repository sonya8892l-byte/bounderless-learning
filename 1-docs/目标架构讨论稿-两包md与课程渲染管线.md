...
> **落地进度（2026-08-07 晚）**：D1／D3／D4／D5／**D6** 已实现——任务单元就地格式定稿、5 门存量课迁移完成、`guidance/`＋`scaffolds/` 目录取消、语义理解优先已接入 `runTurn`、**平台默认层六份 md 全部接入且 `mergeDefaults` 已实现（偏差① 关闭）**。
...
| 十种工具默认参数/提示文案 | `tool-registry.js` | `tool-defaults.md`（中文正则匹配器留代码——它决定分支逻辑，不是文案） |
| 时长/提醒/推进方式等默认值 | `lesson-parser.js` 硬编码 | `defaults.md`（当可选入参传进 `parseLesson`，服务端与浏览器包同源） |

**当时的后果**：课程作者改不动语气与话术；教研调整平台默认要改代码发版；"换课程只需 md"在系统侧不成立。这是对承诺1 最大的违背。**现已解除。**

**目标**：平台包升级为一个完整的**默认层（default layer）**——可以理解为"一门默认课程"：课程包不写的一切都有平台默认，课程包写了的按白名单覆盖。每份平台文件显式声明 `immutable`（三底线 + IP 锁定核）或 `overridable`。
...
flowchart LR
    subgraph SRC["① 源码层：唯二输入"]
      PMD["平台包 _platform/<br/>底线规则（不可覆盖）<br/>+ 默认层：人设/话术/学段/<br/>脚手架策略/工具默认/标签树"]
      CMD["课程包 lesson_xxx/<br/>manifest/阶段/角色+任务单元/<br/>知识/限制/（方法论·预留）"]
    end

    COMP["② 统一编译器<br/>校验（引用·锚点·素材·标签·严格模式）<br/>→ 合并（平台默认 ← 课程覆盖）<br/>→ 装配任务图"]
...
| 要求 | 保证机制 |
|---|---|
| 通用 | 管线零课程特判；换课 = 换课程包（承诺1）；平台默认吃掉所有"课程没写"的情况 |
...
│                                                              ← 现 service.js 散落逻辑
├── tool-defaults.md             [overridable] 十种活动工具的默认参数与提示文案
│                                                              ← 现 tool-registry.js 部分
├── defaults.md                  [overridable] 时长/无操作提醒/提醒冷却/推进方式等默认值
...
│                        推进模式(新,默认sequential)/人设侧重(真正生效)/视觉素材
...
| 概念 | 是什么 | 在哪声明 |
|---|---|---|
| **任务图**（数据） | 任务节点 + `前置` 依赖关系 | 课程包任务单元的 `前置` 字段；默认空 = 链式（承接上一任务） |
| **遍历模式**（策略） | `sequential / open / inquiry` | `course.md` 的 `推进模式`（可按 Phase 覆盖）；默认 sequential |
...
轨道一 · 内容与编译（决定 md 怎么写，规范先行——课程团队每天在产内容）
  ...
  M1  编译器改造：统一编译器 → Course IR → 三投影
      ...
      兼容：双格式期——旧布局照常编译并告警
  M2  平台默认层落地：companion/voice/language-levels/scaffolding/
...
| D2 | 进度语义的权威字段 | **`前置`（入边）为权威**——同时表达顺序与解锁，天然支持模式2；`通过后` 保留为线性课程的语法糖，编译期转换为前置关系 | 🟡 **编译期转换已实现**（M1-A2，`server/course/task-graph.js`）：**87 个角色任务节点**、29 个终止节点、零告警；显式 `前置` 优先于语法糖推导。图里另有**阶段任务节点**（非角色任务，键 `phaseId/taskId`，不带 roleId 因此不进角色遍历，见 P 系列）。**两类都仍无运行时读者**——推进仍是 `currentTaskIndex += 1`，换成读图是 R3 |
| D3 | 就地/集中的边界 | **就地**：引导、脚手架、Step 验收标准；**集中**：知识、限制、课程级量规、时间银行 | ✅ 含 B4 根治 |
| D4 | 平台覆盖白名单 | **不可覆盖**：三底线、絮絮名字/底色；**可覆盖**：语气侧面、话术模板、学段规范、工具默认、时长默认 | ✅ 已实现（M2，2026-08-07）：六份默认层 md ＋ `mergeDefaults` ＋ 声明块（`overridable`/`merge`/`course-field`/`locked`）；被拦下的覆盖产出 warning 不静默丢弃 |
...
1. **理解**（轻量 LLM，语言输入必经）：`understandTurn(text, contextSlice)` → 结构化 JSON `{ 意图, 情绪, 是否在答待答问题, 想要什么, 置信度 }`。由现有"回合理解器"（待答问题时的 jsonMode 调用）升级而来；zod 校验＋重试一次＋保守默认（先接住这句话＋温和追问）的降级链，**绝不回落会循环的正则分支**。非语言输入（工具结果、位置、心跳、带问题ID按钮）不经过此步。
