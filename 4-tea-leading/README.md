...
访问 `http://127.0.0.1:3000/teacher/`。本地演示环境不需要凭证；首次进入会创建一个 30 人、5 个学习小组的故宫演示场次。

预览与正式环境由服务器要求 Bearer 鉴权。教师在安全入口页手动输入管理员发放的访问凭证；凭证只保存在当前标签页的 `sessionStorage`，不进入静态 JS、URL、日志或页面文案。所有教师 API 与地图配置请求经 `teacher-auth.js` 统一加上 `Authorization: Bearer ...`。

可以配置多套教师凭证（环境变量 `TEACHER_ACCOUNTS`），每套对应独立的 `teacherId` 与场次数据。带 `experiencePack` 的账号第一次列出场次时，服务端会为**每门已发布课程**各建一场 1 名体验学生的小场次；课程内容相同，进度、证据和指令互不影响。教师在场次卡片上复制该课的专属学习链接即可。旧的单凭证 `TEACHER_API_TOKEN` 仍然可用，且默认不会自动建体验场次。

- `401`：当前会话凭证会被清理，页面回到凭证输入态。
- `503`：页面进入服务器配置态，明确提示联系管理员，不显示或记录凭证。
- 浏览器 WebSocket 无法安全设置 Bearer header。有鉴权会话时，教师端使用现有的 5 秒轮询通道，不把凭证降级放入 URL query 或 WebSocket subprotocol。

教师通过课前检查名单或学生详情抽屉，可以逐人复制专属学习链接或显示本地生成的二维码。学生入课凭证放在 URL fragment（`#...`），首个 HTTP 请求、CDN 访问日志和 Referer 不会收到该值；学生页读入后立即转存当前标签页的 `sessionStorage` 并清除 fragment。教师快照缓存前会深度剔除两处学生入课凭证，剩余快照只进入 `sessionStorage`，401 或结束会话时清理。二维码在浏览器本地编码，不把链接发往第三方图片服务。

正式环境第一次登录且尚无场次时，页面保留「创建场次」入口，不调用已禁用的 demo 初始化。新场次默认为草稿，课前检查后需在教学遥控器中二次确认「正式开始场次」，才会激活学生学习会话。

## 已实现

- 场次中心：创建多课程场次、CSV 名单导入与分组；学生在教师开放窗口领取角色，同组角色保持唯一。
...
- **人员级身份体系**：预览/正式环境已由服务端 Bearer 门禁与教师 PWA 安全会话保护。当前支持多套教师共享密钥（`TEACHER_ACCOUNTS`），按 `teacherId` 隔离场次；若要区分每名教师的登录账号、主动过期和撤销单个会话，还需接入组织登录与授权服务。
