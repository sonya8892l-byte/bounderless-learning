...
可以配置多套教师凭证（环境变量 `TEACHER_ACCOUNTS`），每套对应独立的 `teacherId` 与场次。带 `experiencePack` 的账号第一次列出场次时，服务端会自动建一个 1 名体验学生的小场次；教师在课前检查里复制该学生的专属学习链接即可。旧的单凭证 `TEACHER_API_TOKEN` 仍然可用，且默认不会自动建体验场次。
